...
router.post('/submit_morepicture',upload.any(), function(req, res, next) {
  try {
    var  files=req.files.map((item)=>{
        return item.filename
    })
    pool.query('insert into morepictures(categoryid, fooditemid, picture, createdate, createtime, userid) values(?,?,?,?,?,?)',[req.body.categoryid, req.body.fooditemid, files+"", req.body.createdate, req.body.createtime, req.body.userid],function(error,result){
        if(error){ 
            res.status(500).json({status:false,message:'database error please contact with backend team'})
            console.log(error)
        }else{
