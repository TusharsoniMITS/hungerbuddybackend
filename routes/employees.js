...
router.post('/edit_picture_employee',upload.single('employee_picture'), async function(req, res, next) {
  try {
        const imageUrl = await uploadToCloudinary(req.file.path);

    pool.query('update employee set employee_picture=?, createddate=?, createdtime=?, userid=?  where employeeid=?',[imageUrl, req.body.createddate, req.body.createdtime, req.body.userid, req.body.employeeid],function(error,result){
        if(error){ 
            res.status(500).json({status:false,message:'database error please contact with backend team'})
            console.log(error)
        }else{
